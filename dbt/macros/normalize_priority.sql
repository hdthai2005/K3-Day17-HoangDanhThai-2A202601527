...
{% macro priority_reject_reason(col) %}
    case
        when {{ col }} is null or {{ col }} = '' then 'priority bị trống'
        when try_cast({{ col }} as integer) is not null and try_cast({{ col }} as integer) not in (1, 2, 3, 4) then 'priority là số nhưng ngoài khoảng 1..4'
        else 'priority không hợp lệ hoặc chuỗi lạ'
    end
{% endmacro %}
